...
std::vector<std::tuple<long, unsigned long, unsigned long>> Example3()
{
    File_stream<float> file_stream;
    auto input_tensor1 = Generate<float>::placeHolder(
        Shape(5, 4, 3), file_stream, "test placeHolder operation0");
    auto input_tensor2 =
        Generate<float>::weight(Shape(4, 5, 3), true, "test weight operation0");

    auto input_tensor3 =
        Generate<float>::weight(Shape(5, 5, 3), true, "test weight operation1");

    auto multiplied_tensor1 = Operate<float>::matMul(
        input_tensor1, input_tensor2, "matMul operation1");  // shape: 5,5,3

    auto multiplied_tensor2 =
        Operate<float>::matMul(multiplied_tensor1, input_tensor3,
                               "matMul operation2");  // shape: 5,5,3

    auto added_tensor1 =
        Operate<float>::matAdd(multiplied_tensor2, input_tensor3,
                               "matMul operation3");  // shape: 5,5,3

    auto reshaped_tensor1 = Operate<float>::reshape(
        added_tensor1, Shape(25, 1, 1), "reshape_operation1");

    Final<float>::wrapper(reshaped_tensor1, "wrapper_operation1");
...
std::vector<std::tuple<long, unsigned long, unsigned long>> Example2()
{
    ...
    auto input_tensor1 = Generate<int>::placeHolder(
        Shape(2, 4, 3), file_stream, "test placeHolder operation");  // 1
    ...
    Final<int>::wrapper(multiplied_tensor1, "test wrapper operation0");
...
std::vector<std::tuple<long, unsigned long, unsigned long>> Example1()
{
    ...
    auto dot_operated_tensor1 =
        Operate<int>::matDot(added_tensor1, 5, "test matDot operation");  // 7
