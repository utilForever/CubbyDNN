...
namespace cubby_dnn {
    template<typename T>
    Tensor<T>::Tensor(std::vector<T> &data, std::vector<int> &shape, TensorType type){
    ...
    template<typename T>
    Tensor<T>::Tensor(std::vector<T> &data, std::vector<int> &shape, TensorType type, std::string name) {
        //TODO: Throw appropriate exception if data size and shape doesn't match
        this->Data = std::make_unique<storage<T>>(std::forward<std::vector<T>>(data),
                                                  std::forward<std::vector<int>>(shape));
        this->name = name;
