...
namespace cubby_dnn{
    ...
    template<typename T>
    Mat_mul_op<T>::Mat_mul_op(std::shared_ptr<Tensor<T>> tensor1, std::shared_ptr<Tensor<T>> tensor2,
    ...
    template<typename T>
    Mat_add_op<T>::Mat_add_op(std::shared_ptr<Tensor<T>> tensor1, std::shared_ptr<Tensor<T>> tensor2,
            std::shared_ptr<Tensor<T>> output_tensor){
        this->input_tensor_vect.emplace_back(tensor1);
        this->input_tensor_vect.emplace_back(tensor2);
        this->output_tensor_vect.emplace_back(output_tensor);
    ...
    template<typename T>
    Mat_add_op<T>::Mat_add_op(std::vector<std::shared_ptr<Tensor<T>>> tensor_vect, std::shared_ptr<Tensor<T>> output_tensor) {
        for(std::shared_ptr<Tensor<T>> ptr: tensor_vect){
            this->input_tensor_vect.emplace_back(ptr);
        }
    ...
    template<typename T>
    Mat_dot_op<T>::Mat_dot_op(std::shared_ptr<Tensor <T>> tensor1, std::shared_ptr<Tensor<T>> identity_tensor,
            std::shared_ptr<Tensor<T>> output_tensor) {
        this->input_tensor_vect.emplace_back(tensor1);
    ...
    template<typename T>
    Reshape_op<T>::Reshape_op(std::shared_ptr<Tensor<T>> tensor1, std::shared_ptr<Tensor<T>> output_tensor) {
        this->input_tensor_vect.emplace_back(tensor1);
        this->output_tensor_vect.emplace_back(output_tensor);
    }

    template<typename T>
    placeHolder<T>::placeHolder(const std::vector<int> &shape, std::shared_ptr<Tensor<T>> output_tensor) {
        this->output_tensor_vect.emplace_back(output_tensor);
    }

    template<typename T>
    weight<T>::weight(const std::vector<int> &shape, std::shared_ptr<Tensor<T>> output_tensor) {
        this->output_tensor_vect.emplace_back(output_tensor);
    }
}
