...
#ifndef CAPTAIN_SPINLOCKQUEUE_HPP
#define CAPTAIN_SPINLOCKQUEUE_HPP

#include <captain/Engine/SpinLockQueue-Decl.hpp>
...
SpinLockQueue<T>::SpinLockQueue(SpinLockQueue<T> &&spinLockQueue) noexcept
    : m_maxCapacity(spinLockQueue.m_maxCapacity),
      ...
      m_backIndex(spinLockQueue.m_backIndex), m_empty(spinLockQueue.m_empty) {}
...
template <typename T> T SpinLockQueue<T>::Dequeue() {
  while (m_empty)
    ;
  auto rtn = TryDequeue();
  while (!std::get<1>(rtn)) {
...
template <typename T> std::tuple<T, bool> SpinLockQueue<T>::TryDequeue() {
...
template <typename T> T &SpinLockQueue<T>::At(size_t index) {
  // assert(index > size());
...
template <typename T> size_t SpinLockQueue<T>::Size() {
  m_spinLock.SharedLock();
...
template <typename T> void SpinLockQueue<T>::incrementBack() {
...
template <typename T> void SpinLockQueue<T>::incrementFront() {
